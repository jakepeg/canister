...
function getLocalIdentityProvider(): string {
  ...
  if (isSafariLikeBrowser) {
    return `http://localhost:4943/?canisterId=${localIiCanisterId}`;
  }

  return `http://${localIiCanisterId}.localhost:4943/`;
}

const DEFAULT_IDENTITY_PROVIDER = (() => {
  if (process.env.DFX_NETWORK === "local") {
    return getLocalIdentityProvider();
  }

  // Prefer explicit II_URL (in non-local envs), otherwise default to hosted II.
  if (process.env.II_URL && process.env.II_URL.length > 0) {
...
// Align with the working app: no explicit derivationOrigin in the default flow.
const DEFAULT_DERIVATION_ORIGIN = undefined;
...
async function createAuthClient(
  ...
  const options: AuthClientCreateOptions = {
    ...
    loginOptions: {
      derivationOrigin:
        config.ii_derivation_origin ?? DEFAULT_DERIVATION_ORIGIN,
    },
    ...createOptions,
  };
...
export function InternetIdentityProvider({
  ...
  const login = useCallback(() => {
    ...
    const options: AuthClientLoginOptions = {
      ...
      derivationOrigin: DEFAULT_DERIVATION_ORIGIN,
